...
public class PanelMenuPrinAdministrador extends JPanel implements Colores, Fonts, ConstantesComponentes {

	private static final long serialVersionUID = 1L;
	private Listener listener;

public PanelMenuPrinAdministrador() {
		...
		JLabel txtInfoCuenta = new JLabel("Usuarios");
		...
		JPanel panelIconoEnfermedad = new JPanel();
		panelIconoEnfermedad.setBounds(0, 196, 55, 39);
		panelIconoEnfermedad.setBackground(COLOR_BARRA_LATERAL);
		panelIconos.add(panelIconoEnfermedad);
		panelIconoEnfermedad.setLayout(new BorderLayout(0, 0));
		
		JLabel iconoEnfermedad = new JLabel("");
		iconoEnfermedad.setHorizontalAlignment(SwingConstants.CENTER);
		iconoEnfermedad.setIcon(new ImageIcon(PanelMenuPrinAdministrador.class.getResource("/com/imagenes/enfermedad.png")));
		panelIconoEnfermedad.add(iconoEnfermedad, BorderLayout.CENTER);
		...
		panelEnfermedades.setBounds(0, 196, 210, 39);
		panelEnfermedades.setBackground(COLOR_PRINCIPAL);
		ContenidoPanel.add(panelEnfermedades);
		panelEnfermedades.setLayout(new BorderLayout(0, 0));
		
		JLabel txtEnfermedades = new JLabel("Enfermedades");
		txtEnfermedades.setFont(FUENTE_PRINCIPAL);
		txtEnfermedades.setForeground(COLOR_FONDO);
		txtEnfermedades.setHorizontalAlignment(SwingConstants.CENTER);
		panelEnfermedades.add(txtEnfermedades, BorderLayout.CENTER);
		...
		panelEnfermedades.addMouseListener(new MouseAdapter() {
			
			public void mouseEntered(MouseEvent e) {
				resaltar(txtEnfermedades, panelEnfermedades);
				actualizarCursor(panelEnfermedades);
			}
			
			public void mouseExited(MouseEvent e) {
				restaurar(txtEnfermedades, panelEnfermedades);
				resetearCursor(panelEnfermedades);
			}
			
			public void mouseClicked(MouseEvent e) {
				onOptionSelected(ENFERMEDADES);
			}
